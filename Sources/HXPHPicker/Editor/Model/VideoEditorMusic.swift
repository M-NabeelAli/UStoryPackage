...
class VideoEditorMusic: Equatable, Codable {
    ...
    func parseLrc() {
        ...
        if lyrics.isEmpty {
            lyricIsEmpty = true
            lyrics.append(.init(lyric: "There are no lyrics for this song, please enjoy".localized))
        }
    }
    ...
    func lyric(
        ...
        if lyricIsEmpty {
            return .init(lyric: "There are no lyrics for this song, please enjoy".localized)
        }
        for lyric in lyrics {
