           ...
           localIdentifier == photoAsset.phAsset?.localIdentifier {
            ...
            if loadingView == nil {
                loadingView = ProgressHUD.showLoading(
                    addedTo: hudSuperview(),
                    text: "iCloud syncing".localized + "(" + String(Int(photoAsset.downloadProgress * 100)) + "%)",
                    animated: true)
            }
        }else {
...
//        #endif
    ...
    func requestShowDonwloadICloudHUD(iCloudRequestID: PHImageRequestID) {
        ...
        showLoadingView(text: "iCloud syncing".localized)
    }
    ...
    func requestFailed(
        ...
        if let info = info, !info.isCancel {
            ...
            ProgressHUD.hide(forView: hudSuperview(), animated: false)
            if showWarning {
                let text = (info.inICloud && isICloud) ? "iCloud sync failed".localized : "Download failed".localized
