...
class PhotoPickerBottomNumberView: UICollectionReusableView {
    ...
    func configData() {
        ...
        if photoCount > 0 && videoCount > 0 {
            contentLb.text = photoString  + "Photos".localized + "、" + videoString + "Videos".localized
        }else if photoCount > 0 {
            contentLb.text = photoString + "Photos".localized
        }else if videoCount > 0 {
            contentLb.text = videoString + "Videos".localized
        }else {
            contentLb.text = nil
        }
    }
