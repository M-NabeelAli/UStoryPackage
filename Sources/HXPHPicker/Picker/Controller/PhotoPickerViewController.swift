...
public class PhotoPickerViewController: BaseViewController {
    ...
    public lazy var collectionView: UICollectionView = {
        ...
        if config.allowAddText {
            
            collectionView.register(
                PickerAddTextCell.self,
                forCellWithReuseIdentifier:
                    NSStringFromClass(PickerAddTextCell.classForCoder())
            )
        }
    ...
    var textCell: PickerAddTextCell {
        let indexPath: IndexPath
        indexPath = IndexPath(item: 1, section: 0)
        
        let cell = collectionView.dequeueReusableCell(
            withReuseIdentifier: NSStringFromClass(
                PickerAddTextCell.classForCoder()
            ),
            for: indexPath
        ) as! PickerAddTextCell
        cell.config = config.AddTextCell
        return cell
    }
    ...
    var needOffset: Bool {
        if config.sort == .desc {
            if canAddCamera || canAddLimit || canAddText{
                return true
            }
        }
        return false
    ...
    var offsetIndex: Int {
        ...
        var offset = canAddText ? 1 : 0
        offset =  canAddCamera ? offset + 1 : offset
        offset =  canAddLimit ? offset + 1 : offset
        return offset
    }
    ...
    public override func viewDidLayoutSubviews() {
        ...
        collectionViewLayout.itemSize = CGSize.init(width: itemWidth, height: itemWidth * 1.3)
...
extension PhotoPickerViewController {
    ...
    func addedPhotoAsset(for photoAsset: PhotoAsset) {
        ...
        if config.sort == .desc {
            ...
            indexPath = IndexPath(item: needOffset ? offsetIndex : 0,section: 0)
        }else {
