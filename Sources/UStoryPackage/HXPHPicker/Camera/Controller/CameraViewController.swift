...
open class CameraViewController: BaseViewController {
    ...
    func switchCameraFailed() {
        ProgressHUD.showWarning(
            addedTo: view,
            text: "Camera switching failed!".localized,
            animated: true,
            delayHide: 1.5
        )
    }
        ...
        DispatchQueue.global().async {
            do {
                ...
                DispatchQueue.main.async {
                    PhotoTools.showConfirm(
                        viewController: self,
                        title: "Camera initialization failed!".localized,
                        message: nil,
                        actionTitle: "Done".localized
                    ) { _ in
                        self.dismiss(animated: true)
                ...
                if isGranted {
                    ...
                    DispatchQueue.main.async {
                        PhotoTools.showAlert(
                            viewController: self,
                            title: "Cannot use microphone".localized,
                            message: "Please allow access to the microphone in Settings-Privacy-Camera".localized,
                            leftActionTitle: "Cancel".localized,
                        leftHandler: { alertAction in
                            self.addAudioInputFailed()
                        },
                            rightActionTitle: "settings".localized
                        ) { alertAction in
    ...
    func addAudioInputFailed() {
        ProgressHUD.showWarning(
            addedTo: self.view,
            text: "Failed to add microphone, there will be no sound when recording video!".localized,
            animated: true,
            delayHide: 1.5
        )
    }
