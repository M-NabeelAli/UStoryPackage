    ...
    func getAdditiveCell(_ indexPath: IndexPath) -> UICollectionViewCell? {
        if canAddCamera && canAddLimit {
            if config.sort == .asc {
                ...
                if indexPath.item == 1 {
                    return config.allowAddText  ? textCell : limitAddCell
                }
                if indexPath.item == 2 {
                    return config.allowAddLimit ? limitAddCell : textCell
                }
            }
        }else if canAddCamera || canAddLimit || canAddText{
            if config.sort == .asc {
//                if indexPath.item == assets.count {
...
//        }
    ...
    public func collectionView(
        ...
        let photoAsset = getPhotoAsset(for: indexPath.row)
...
extension PhotoPickerViewController: UICollectionViewDelegate {
        ...
        if cell is PickerCameraViewCell {
            if !UIImagePickerController.isSourceTypeAvailable(.camera) {
                ProgressHUD.showWarning(
                    ...
                    text: "Camera is not available!".localized,
                    animated: true,
                    delayHide: 1.5
                )
                return
    ...
    func openEditor(_ photoAsset: PhotoAsset,
                    ...
                    animated: Bool = true,_ allPhotoAsset: [PhotoAsset] = []) {
...
//        #if HXPICKER_ENABLE_EDITOR && HXPICKER_ENABLE_PICKER
        if pickerController.config.editorOptions.contains(.photo) {
            ...
            photoEditorVC.previewAssets = AllPhotoAssets
...
//        #endif
    ...
    public func collectionView(
        ...
        return .init(
            ...
            else if sCell is PickerAddTextCell  {
                let vc = PhotoPeekViewController(isCamera: true)
                return vc
            }
            return nil
            ...
            if picker.config.selectMode == .multiple {
                let select = UIAction(
                    title: photoAsset.isSelected ? "Deselect".localized : "Select".localized,
