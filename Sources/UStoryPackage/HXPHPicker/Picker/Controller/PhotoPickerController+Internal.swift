        ...
        if photoAsset.mediaType == .photo {
            if config.maximumSelectedPhotoFileSize > 0 {
                if photoAsset.fileSize > config.maximumSelectedPhotoFileSize {
                    text = "Photo size exceeds maximum limit".localized + PhotoTools.transformBytesToString(
            ...
            if !config.allowSelectedTogether {
                if selectedVideoAssetArray.count > 0 {
                    text = "Photos and videos cannot be selected at the same time".localized
                    canSelect = false
                }
            }
            if config.maximumSelectedPhotoCount > 0 {
                if selectedPhotoAssetArray.count >= config.maximumSelectedPhotoCount {
                    text = String.init(format: "Only %d photos can be selected at most".localized, arguments: [config.maximumSelectedPhotoCount])
                    canSelect = false
                }
            }else {
                if selectedAssetArray.count >= config.maximumSelectedCount && config.maximumSelectedCount > 0 {
                    text = String.init(format: "Maximum selections reached".localized, arguments: [config.maximumSelectedPhotoCount])
                    canSelect = false
                }
            }
        }
        ...
        if photoAsset.mediaType == .video {
            if config.maximumSelectedVideoFileSize > 0 {
                if photoAsset.fileSize > config.maximumSelectedVideoFileSize {
                    text = "Video size exceeds maximum limit".localized + PhotoTools.transformBytesToString(
...
//                    #if HXPICKER_ENABLE_EDITOR
                    if !config.editorOptions.contains(.video) || filterEditor {
                        text = String(
                            format: "The maximum video duration is %d seconds, which cannot be selected".localized,
                        ...
                        if config.maximumVideoEditDuration > 0 &&
                            ...
                            text = String(
                                format: "The video can be edited for a maximum length of %d seconds and cannot be edited".localized,
...
//                    #else
//                    text = String(
//                        format: "The maximum video duration is %d seconds and cannot be selected".localized,
//                        arguments: [config.maximumSelectedVideoDuration]
//                    )
                    canSelect = false
//                    #endif
            ...
            if config.minimumSelectedVideoDuration > 0 {
                if round(photoAsset.videoDuration) < Double(config.minimumSelectedVideoDuration) {
                    text = String(
                        format: "The minimum length of the video is %d seconds and cannot be selected".localized,
            ...
            if !config.allowSelectedTogether {
                if selectedPhotoAssetArray.count > 0 {
                    text = "Video and photo cannot be selected at the same time".localized
                    canSelect = false
                }
            }
            if config.maximumSelectedVideoCount > 0 {
                if selectedVideoAssetArray.count >= config.maximumSelectedVideoCount {
                    text = String.init(format: "You can only select up to %d videos".localized, arguments: [config.maximumSelectedVideoCount])
                    canSelect = false
                }
            }else {
                if selectedAssetArray.count >= config.maximumSelectedCount && config.maximumSelectedCount > 0 {
                    text = String.init(format: "Maximum number of choices reached".localized, arguments: [config.maximumSelectedPhotoCount])
                    canSelect = false
                }
            }
        }
