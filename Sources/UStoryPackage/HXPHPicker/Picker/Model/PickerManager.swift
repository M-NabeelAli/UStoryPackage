        ...
        if photoAsset.mediaType == .photo {
            if config.maximumSelectedPhotoFileSize > 0 {
                if photoAsset.fileSize > config.maximumSelectedPhotoFileSize {
                    text = "The photo size exceeds the maximum limit".localized + PhotoTools.transformBytesToString(
            ...
            if config.maximumSelectedPhotoCount > 0 {
                if selectedPhotoAssetArray.count >= config.maximumSelectedPhotoCount {
                    text = String.init(format: "You can only select up to %d photos".localized, arguments: [config.maximumSelectedPhotoCount])
                    canSelect = false
                }
            }else {
            ...
            if config.maximumSelectedVideoFileSize > 0 {
                if photoAsset.fileSize > config.maximumSelectedVideoFileSize {
                    text = "Video size exceeds the maximum limit".localized + PhotoTools.transformBytesToString(
...
//                    #if HXPICKER_ENABLE_EDITOR
                    if !config.editorOptions.contains(.video) {
                        ...
                        if config.maximumVideoEditDuration > 0 &&
                            ...
                            text = String(
                                format: "The maximum length of the video that can be edited is %d seconds and cannot be edited".localized,
