...
extension PhotoTools {
    ...
    static func showNotCameraAuthorizedAlert(
        ...
        showAlert(
            viewController: vc,
            title: "Unable to use camera function".localized,
            message: "Please go to system settings and allow access to Camera".localized,
            leftActionTitle: "Cancel".localized,
            leftHandler: { _ in
                cancelHandler?()
            },
            rightActionTitle: "Go to system settings".localized
        ) { (alertAction) in
            openSettingsURL()
        }
    }
